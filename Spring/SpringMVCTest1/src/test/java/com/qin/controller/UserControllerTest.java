...
public class UserControllerTest {
	...
	public void test8() throws IOException {
		RestTemplate restTemplate = new RestTemplate();
		byte[] response = restTemplate.postForObject("http://localhost:8080/SpringMVCTest1/test8/{itemId}", null,
				byte[].class, "1234");
	...
	public void test11() {
		...
		HttpEntity<User> responsEntity = restTemplate.exchange("http://localhost:8080/SpringMVCTest1/test11",
				HttpMethod.POST, requestBody, User.class);

		// 将请求相应消息转为User对象，并对响应值进行判断
		User responseUser = responsEntity.getBody();
		System.out.println(responsEntity);
		AssertJUnit.assertNotNull(responseUser);
		AssertJUnit.assertEquals("1000", responseUser.getuId());
		AssertJUnit.assertEquals("Tom", responseUser.getUserName());
		AssertJUnit.assertEquals("1234", responseUser.getTel());
	...
	public void test12() {
		...
		ListenableFuture<ResponseEntity<User>> future = template
				.getForEntity("http://localhost:8080/SpringMVCTest1/test12", User.class);		
