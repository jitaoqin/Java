...
public class TestAspect {
	...
	@Before("!target(com.smart.NaiveWaiter) "+
			"&& execution(* serveTo(..)))")//匹配所有serveTo方法并且该方法不位于NaiveWaiter目标类的切点
	public void notServeInNaiveWaiter() {
		System.out.println("--notServeInNaiveWaiter() executed!--");
	}
	@After("within(com.smart.*) "
			+ " && execution(* greetTo(..)))")//匹配com.smart包中的所有greetTo方法的切点
	public void greeToFun() {
		System.out.println("--greeToFun() executed!--");
	}
	
	@AfterReturning("target(com.smart.Waiter) || "+
			        " target(com.smart.Seller)")//匹配waiter和seller接口实现类所有了连接点的切点
