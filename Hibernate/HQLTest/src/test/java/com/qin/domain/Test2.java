...
//		 }
//		 
//		 Query query3 = session.createQuery("select  new List(id,name,age) from Customer");
...
//		 }
		/* Query query4 = session.createQuery(" from Commodity c where c.price >= 400 and c.description = null");
		 List<Commodity> commodities = query4.list();
    ...
    /* List<Object[]> results = session.createCriteria(Commodity.class).add(Restrictions.isNotNull("description"))
    		 .add(Restrictions.or(Restrictions.ge("price", 1000.0), Restrictions.eq("price", 400.0)))
    		 .addOrder(Order.desc("price"))
    		 .setProjection(
    				 Projections.projectionList()
    				 	.add(Projections.groupProperty("seller"))
     ...
     for (Object[] object : results) {
		System.out.print(((Seller) object[0]).getName() + " ");
